...
void apply_sprite(SDL_Renderer * renderer, SDL_Texture * texture, sprite_t * sprite){
    if (sprite->is_visible == 0)
    {
        ...
        SrcR.h = sprite->h;
        SrcR.x = sprite->wich_img[0]; //* (SPACE_BETWEEN_COLUMNS + sprite->w[sprite->wich_img[0]]); marche pas car largeur différentes pour chaque colonne
...
void apply_resized_sprite(SDL_Renderer * renderer, SDL_Texture * texture, sprite_t * sprite, int f){
    if (sprite->is_visible == 0)
    {
        sprite->h /= sprite->resize; //On remets les données du sprite à leur valeur originelle pour que le sprite s'affiche correctement quand c'est un sprite sheet
        sprite->w[sprite->wich_img[0]] /= sprite->resize; //Idem
        ...
        sprite->h *= sprite->resize; //On remet les données du sprite à leur valeur après avoir redimensionner le sprite pour que ses données soient équivalentes à son affichage
        sprite->w[sprite->wich_img[0]] *= sprite->resize; //Idem
    }
}
...
void  init_resources(SDL_Renderer *renderer, resources_t *resources){
    ...
    resources->player = load_transparent_image("ressources/player_sprite_sheet.bmp", renderer,230,80,235);
...
void refresh_graphics(SDL_Renderer *renderer, world_t *world,resources_t *resources){
    ...
    apply_resized_sprite(renderer, resources->player, world->player->sprite, 2);
